			...
			if (status == 200) {
				...
				for (int i = 0; i < postsArrayResponse.length(); i++) {
					...
					if ("text".equals(type)) {
						...
						TextPost newPost = new TextPost(id, type, timestamp,
								title, body);
						result.add(newPost);
						...
						for (int j = 0; j < photoArray.length(); j++) {
							JSONObject photo = photoArray.getJSONObject(j).getJSONObject("original_size");
							String url = photo.getString("url");
							photos.add(url);
						}
						
						PhotoPost newPost = new PhotoPost(id, type, timestamp,
								caption, photos);
						result.add(newPost);
					} else if ("quote".equals(type)) {
						String text = Html.fromHtml(post.getString("text")).toString();
						String source = post.getString("source");

						QuotePost newPost = new QuotePost(id, type, timestamp,
								text, source);
						result.add(newPost);
						...
						LinkPost newPost = new LinkPost(id, type, timestamp,
								title, url);
						result.add(newPost);
						...
						String caption = Html.fromHtml(post.getString("caption")).toString();
						...
						String question = Html.fromHtml(post.getString("question")).toString();
						String answer = post.getString("answer");

						AnswerPost newPost = new AnswerPost(id, type,
								timestamp, askingName, askingUrl, question,
								answer);
						result.add(newPost);
					} else {

						Post newPost = new Post(id, type, timestamp);
						result.add(newPost);
					}
				}
			}
